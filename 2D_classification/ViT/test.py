...
def test_model(args):
    ...
    if dataset_type == "flower":
        ...
        valDataloader = DataLoader(valDataset, batch_size=batch_size, shuffle=True,
                                    pin_memory=True, collate_fn=valDataset.collate_fn)
    ...
    model = ViT(classNum=class_num).to(device)
    ...
    for imgs, _ in valDataloader:
        ...
        with torch.no_grad():
            ...
            plt.tight_layout()
            plt.show()
            plt.savefig('img/vit_{}2.png'.format(dataset_type), bbox_inches='tight')
            break
