...
def room2blocks(room_xyz, offset):
    ...
    block_locations, point_block_indices, block_point_counts = np.unique(
        np.floor((room_xyz - room_xyz_min) / block_size).astype(np.int_),
...
def merge_small_blocks(block_locations, block_point_indices):
    ...
    for block_idx in range(len(block_locations)):
        ...
        for x, y in nbr_block_offsets:
            ...
            block_point_indices[block_idx] = np.array([], dtype=np.int_)
            break
...
def block2grids(block_xyz):
    ...
    grid_locations, point_grid_indices, grid_point_counts = np.unique(
        np.floor((block_xyz - block_xyz_min) / args.grid_size).astype(np.int_),
        return_inverse=True, return_counts=True, axis=0)
...
def main():
    input_root = 'Stanford3dDataset_v1.2_Aligned_Version'
    output_root = 'S3DIS_npz'
    ...
    for area_name in sorted(list(os.walk(input_root))[0][1]):
        os.makedirs(os.path.join(output_root, area_name), exist_ok=True)
        area_dir = os.path.join(input_root, area_name)
        for room_name in sorted(list(os.walk(area_dir))[0][1]):
            ...
            room_path = os.path.join(output_root, area_name, '%s.npz' % room_name)
            print("Room path is {}".format(room_path))
            ...
            room_path = os.path.join(output_root, area_name, '%s_resampled.npz' % room_name)
