...
def train_model(args):
    ...
    for i in range(args.epochs):
        ...
        for batch in pbar:
            ...
            batch = img.shape[0]
            valid = Variable(FloatTensor(batch, ).fill_(1.0), requires_grad=False)
            fake = Variable(FloatTensor(batch, ).fill_(0.0), requires_grad=False)
            
            D_opt.zero_grad()
            validity_real = D(img, label)
            d_real_loss = loss_func(validity_real.flatten(), valid)
            ...
            d_fake_loss = loss_func(validity_fake.flatten(), fake)
            ...
            g_loss = loss_func(validity.flatten(), valid)
            g_loss.backward()
            G_opt.step()
