...
__global__ void ball_query_kernel(
    ...
    int64_t* __restrict__ batched_index) {
  ...
  int64_t* __restrict__ all_index = batched_index + bid * n_centroids * n_points_per_group;

  float radius_square = radius * radius;
  for (int i = tid; i < n_centroids; i += block_size) {
    int64_t* __restrict__ index = all_index + i * n_points_per_group;
...
void ball_query_kernel_wrapper(const float* batched_points_xyz, 
  ...
  ball_query_kernel<<<n_batches, 1024>>>(batched_points_xyz,
                                         batched_centroids_xyz, n_points,
                                         n_centroids, radius,
                                         n_points_per_group, batched_index);
}