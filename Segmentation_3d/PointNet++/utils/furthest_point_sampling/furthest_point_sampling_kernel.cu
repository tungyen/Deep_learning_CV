...
// This function find maximum value from a list of size up to 64.
// Return result as max_values[0].
template <unsigned int block_size>
__device__ void warpReduce(volatile float* max_values,
                           volatile int* max_values_idx, int tid) {
...
__global__ void furthest_point_sampling_kernel(
  ...
  for (int s = 0; s < n_samples; s++) {
    ...
    float y1 = points_xyz[current_furthest_idx * 3 + 1];
    float z1 = points_xyz[current_furthest_idx * 3 + 2];
    ...
    __syncthreads();
    current_furthest_idx = argmax<block_size>(max_dists, max_dists_idx);
  }
}
